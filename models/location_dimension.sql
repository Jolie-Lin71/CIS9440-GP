...
location_fe AS(
    SELECT borough, community_board, incident_zip, street_name
    FROM base_fe
),
loc_fe_transformed AS(
    SELECT borough AS city_borough,
        CASE 
...
location_ri AS(
    SELECT boro, community_board, zipcode, street
    FROM base_ri
),
loc_ri_transformed AS (
    SELECT  UPPER(boro) AS city_borough,
            CASE 
...
fe_ri_trans_combine AS(
SELECT *
FROM loc_fe_transformed
UNION DISTINCT
SELECT *
FROM loc_ri_transformed
)
SELECT ROW_NUMBER() OVER() AS location_dim_id,
        *
FROM fe_ri_trans_combine
ORDER BY city_borough
